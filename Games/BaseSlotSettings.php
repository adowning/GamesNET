...
class BaseSlotSettings
{
    ...
    public function __construct($settings)
    {
        ...
        $reelStripsData = $settings['reelStrips'];
        // Log::info('DEBUG: Original reelStripsData: ' . json_encode($reelStripsData));
        $reelStrings = [];

        if (is_array($reelStripsData)) {
            foreach ($reelStripsData as $key => $values) {
                if (is_array($values)) {
                    // Convert array of values to comma-separated string
                    $reelStrings[] = $key . '=' . implode(',', $values);
                    // Log::info('DEBUG: Converted ' . $key . ' to ' . $key . '=' . implode(',', $values));
                } else {
                    // Keep as-is if already a string
                    $reelStrings[] = $values;
                    // Log::info('DEBUG: Kept ' . $key . ' as-is: ' . $values);
                }
            }
        }

        // Log::info('DEBUG: Final reelStrings: ' . json_encode($reelStrings));
        $reel = new GameReel($reelStrings);
        foreach (
            [
                'reelStrip1',
                'reelStrip2',
