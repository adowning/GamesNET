...
class User
{
    ...
    {
        // In stateless architecture, save() marks the model for state return
        // The actual persistence happens on the calling server
        if ($this->isModified) {
            $this->changedData['id'] = $this->id; // Ensure ID is included
        }
    }
    ...
    {
        // Return complete current state (for initial load or full sync)
        return [
            'id' => $this->id,
    ...
    {
        // Reset to original state (useful for retry scenarios)
